...
import { Activity, dbApi, initDB, Tweet, User, compatSpamUser } from '$lib/db'
...
describe('spam user', () => {
  ...
  it('compatSpamUser(undefined) 不崩溃且返回默认值', () => {
    const result = compatSpamUser(undefined)
    expect(result).not.undefined
    expect(result.source).toBe('cloud_spam')
    expect(result.action).toBe('hide')
    expect(result.hideStatus).toBe('active')
  })

  it('upsertCloudSpam 首次写入成功', async () => {
    await dbApi.spamUsers.upsertCloudSpam([{ userId: 'first_cloud_user', handle: 'cloud_h' }])
    const res = await dbApi.spamUsers.isSpam(['first_cloud_user'])
    expect(res).toEqual(['first_cloud_user'])
  })

  it('upsertManualBlock 首次写入成功', async () => {
    await dbApi.spamUsers.upsertManualBlock({ id: 'first_manual_user', screen_name: 'manual_h', name: 'manual_n' } as User)
    const res = await dbApi.spamUsers.isSpam(['first_manual_user'])
    expect(res).toEqual(['first_manual_user'])
  })

  it('remove 后 isSpam 不返回该 userId', async () => {
    await dbApi.spamUsers.upsertManualBlock({ id: 'remove_test_user', screen_name: 'rem', name: 'rem_n' } as User)
    expect(await dbApi.spamUsers.isSpam(['remove_test_user'])).toEqual(['remove_test_user'])
    await dbApi.spamUsers.remove('remove_test_user')
    expect(await dbApi.spamUsers.isSpam(['remove_test_user'])).toEqual([])
  })

  it('removeCloudSpamUser 只移除云端来源，不影响手动 block', async () => {
    await dbApi.spamUsers.upsertCloudSpam([{ userId: 'cloud-only', handle: 'cloud' }])
    await dbApi.spamUsers.upsertManualBlock({
      id: 'manual-only',
      screen_name: 'manual',
      name: 'manual',
    } as User)
    await dbApi.spamUsers.removeCloudSpamUser('cloud-only')
    await dbApi.spamUsers.removeCloudSpamUser('manual-only')
    expect(await dbApi.spamUsers.isSpam(['cloud-only', 'manual-only'])).toEqual([
      'manual-only',
    ])
  })

  it('replaceCloudSpamSnapshot 会覆盖云端快照但保留手动 block', async () => {
    await dbApi.spamUsers.upsertCloudSpam([{ userId: 'old-cloud', handle: 'old' }])
    await dbApi.spamUsers.upsertManualBlock({
      id: 'manual-kept',
      screen_name: 'manual',
      name: 'manual',
    } as User)
    await dbApi.spamUsers.replaceCloudSpamSnapshot([
      { userId: 'new-cloud', handle: 'new' },
    ])
    expect(
      await dbApi.spamUsers.isSpam(['old-cloud', 'new-cloud', 'manual-kept']),
    ).toEqual(['new-cloud', 'manual-kept'])
  })
})
