...
describe('sharedSpamFilter', () => {
  it('sharedSpamFilter 命中后返回 hide，不返回 block', () => {
    spamContext.spamUsers.clear()
    spamContext.spamUsers.add('12345')
    const filter = sharedSpamFilter()
    expect(filter.userCondition!({ id: '12345' } as User)).toBe('hide')
    expect(filter.userCondition!({ id: '67890' } as User)).toBe('next')
  })
})
